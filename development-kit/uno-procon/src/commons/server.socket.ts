...
async function nextPlayerAction(
  ...
  if (isChallenge === undefined) {
    ...
    if (isChallenge) {
      ...
      // チャレンジを行わなかった時。引き続き現在のプレイヤーの手番なので、何も処理を行わない。
      return;
    }
  }
...
async function handleDrawCard(
  ...
  if (
    ...
    desk.beforePlayer = player;
    desk.noPlayCount++; // 盤面に動きがない状態なので硬直ターン数を加算
    ...
    async (data: OnPlayCard, callback: CallbackWithData<any>) => {
      ...
      async function execute() {
        ...
        try {
          ...
          if (validateErr) {
            // card_playフィールドのバリデーションエラーなのでペナルティ
            await handlePenalty(
              socket,
              player,
              desk,
          ...
          if (data.yell_uno === undefined || data.yell_uno === null) {
            // yell_unoフィールドが足りないまたはnullなのでペナルティ
            await handlePenalty(
              socket,
              player,
              desk,
          ...
          if (
            ...
            await activityService.create({
              ...
              desk: CommonService.deskLog({
                ...desk,
                beforeCardPlay: { ...desk.beforeCardPlay, color: 'black' }, // desk情報更新済なので更新前の情報に上書き
              }),
            } as any);
            // 場札の色が変更されたことを全体に通知
      ...
      async function execute() {
        ...
        try {
          ...
          if (data.is_play_card === undefined || data.is_play_card === null) {
            // is_play_cardフィールドが足りないまたはnullなのでペナルティ
            await handlePenalty(
              socket,
              player,
              desk,
      ...
      async function execute() {
        ...
        try {
          ...
          if (data.is_challenge === undefined || data.is_challenge === null) {
            // is_challengeフィールドが足りないまたはnullなのでペナルティ
            await handlePenalty(
              socket,
              player,
              desk,
