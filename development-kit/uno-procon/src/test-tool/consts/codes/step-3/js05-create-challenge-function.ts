...
function selectChangeColor() {
  ...
  // TODO 変更する色を判定する実装を自身で自由に記述してください。
}
...
function isChallenge() {
  // TODO チャレンジを行うかを判定する実装を自身で自由に記述してください。
}
/** 追加 ここまで */
  ...
  receiveEvent(SocketConst.EMIT.NEXT_PLAYER, dataRes, () => {
    ...
    if (playCard) {
      ...
      sendEvent(SocketConst.EMIT.DRAW_CARD, {}, (res) => {
        ...
        const playCard = res.draw_card[0]; // 引いたカード。draw-cardイベントのcallbackデータは引いたカードのリストなので、配列の先頭を指定。
